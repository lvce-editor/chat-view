...
test('getStatusBarVirtualDom should render ordered list from assistant message text', () => {
  ...
  const orderedList = result.find((node) => node.type === VirtualDomElements.Ol)
  const listItems = result.filter((node) => node.type === VirtualDomElements.Li)
  const workspaceUriReason = result.find(
    (node) => node.text === 'functions.get_current_workspace_uri - Get the absolute file URI for the currently open workspace folder root.',
  )
  const readFileReason = result.find(
    (node) =>
      node.text ===
      'functions.read_file - Read UTF-8 text content from a file inside the currently open workspace folder. Requires an absolute file URI.',
  )
  ...
  expect(workspaceUriReason).toBeDefined()
