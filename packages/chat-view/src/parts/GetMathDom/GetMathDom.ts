...
const renderMath = async (cacheKey: string, method: string, node: MessageMathInlineNode | MessageMathBlockNode): Promise<void> => {
  try {
...
const getMathDom = (method: string, node: MessageMathInlineNode | MessageMathBlockNode): readonly VirtualDomNode[] | undefined => {
  ...
  if (!pending.has(cacheKey)) {
    ...
    void renderMath(cacheKey, method, node)
  }
  return undefined
}

export const getMathInlineDom = (node: MessageMathInlineNode, _useChatMathWorker = false): readonly VirtualDomNode[] => {
...
export const getMathBlockDom = (node: MessageMathBlockNode, _useChatMathWorker = false): readonly VirtualDomNode[] => {
