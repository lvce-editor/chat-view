...
export const getSessionClassName = (focused: boolean, showFocusOutline: boolean, unread = false): string => {
  ...
  if (showFocusOutline) {
    return mergeClassNames(ClassNames.ChatListItem, ClassNames.ChatListItemFocused, ClassNames.ChatListItemFocusOutline, ClassNames.FocusOutline, unreadClassName)
  }
  if (focused) {
