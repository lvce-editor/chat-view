...
export const test: Test = async ({ Chat, Command, expect, FileSystem, Locator, Workspace }) => {
  ...
  const tsKeyword0 = tsKeywords.nth(0)
  await expect(tsKeyword0).toHaveText('interface')
  const tsKeyword1 = tsKeywords.nth(1)
  await expect(tsKeyword1).toHaveText('readonly')
  const tsKeyword2 = tsKeywords.nth(2)
  await expect(tsKeyword2).toHaveText('string')
  const tsKeyword3 = tsKeywords.nth(3)
  await expect(tsKeyword3).toHaveText('const')
  const tsKeyword4 = tsKeywords.nth(4)
  await expect(tsKeyword4).toHaveText('as')
  const tsKeyword5 = tsKeywords.nth(5)
  await expect(tsKeyword5).toHaveText('const')
