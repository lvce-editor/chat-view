...
export const test: Test = async ({ Chat, Command, expect, Locator }) => {
  ...
  await expect(message1).toContainText('OpenAI request failed (status 400): invalid_value [invalid_request_error].')
  await expect(message1).toContainText("Invalid value: 'input_text'. Supported values are: 'output_text' and 'refusal'.")
}
