...
export const test: Test = async ({ Chat, expect, FileSystem, Locator, Workspace }) => {
  ...
  await expect(message1).toContainText('list_files')
  await expect(message1).toContainText('read_file index.js')
  const message2 = messages.nth(2)
  await expect(message2).toContainText('const mysecretApiKey = "123456";')
  await expect(message2).toContainText('Should I go ahead and update the file with this change?')
  ...
  await expect(message4).toContainText('get_workspace_uri')
  await expect(message4).toContainText('write_file index.js')
  const message5 = messages.nth(5)
  await expect(message5).toContainText('The index.js file has been updated to use const.')
