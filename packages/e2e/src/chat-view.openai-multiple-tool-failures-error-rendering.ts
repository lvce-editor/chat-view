...
export const test: Test = async ({ Chat, Command, expect, Locator }) => {
  ...
  await expect(message0).toHaveText('please create a small tetris game in the current workspace')
  const message1 = messages.nth(1)
  await expect(message1).toContainText('OpenAI request failed.')

  await expect(message1).toContainText('get_workspace_uri')
  await expect(message1).toContainText('create_directory tetris')
  await expect(message1).toContainText('(error: TypeError: fileSystem.mkdir is not a function)')
  await expect(message1).toContainText('create_directory tetris')
  await expect(message1).toContainText("(error: TypeError: Cannot read properties of undefined (reading 'invoke'))")
  const toolCalls = messages.nth(1).locator('.ChatOrderedListItem')
  await expect(toolCalls).toHaveCount(4)
  const toolCall1 = toolCalls.nth(1)
  await expect(toolCall1).toHaveAttribute('title', 'html:///html-playground-2/tetris')
  const toolCall2 = toolCalls.nth(2)
  await expect(toolCall2).toHaveAttribute('title', 'file:///tetris')
  await expect(message1).toContainText('write_file index.html')
  await expect(message1).toContainText(
  ...
  await expect(message1).not.toContainText('+0')
  await expect(message1).not.toContainText('-0')
}
