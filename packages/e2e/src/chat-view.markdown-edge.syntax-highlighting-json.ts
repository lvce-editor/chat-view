...
export const test: Test = async ({ Chat, Command, expect, FileSystem, Locator, Workspace }) => {
  ...
  const jsonProperty3 = jsonProperties.nth(3)
  await expect(jsonProperty3).toHaveText('"value"')
  ...
  const jsonKeyword0 = jsonKeywords.nth(0)
  await expect(jsonKeyword0).toHaveText('true')
  const jsonKeyword1 = jsonKeywords.nth(1)
  await expect(jsonKeyword1).toHaveText('null')
