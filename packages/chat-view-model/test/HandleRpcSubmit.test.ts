...
const registerMockChatToolRpc = (): ReturnType<typeof ChatToolWorker.registerMockRpc> => {
  return ChatToolWorker.registerMockRpc({
...
test('handleRpcSubmit filters disabled tools and plan mode tools before sending to coordinator', async () => {
  using _mockStorageRpc = registerMockChatStorageRpc()
  using mockToolRpc = registerMockChatToolRpc()
  using mockCoordinatorRpc = ChatCoordinatorWorker.registerMockRpc({
    'ChatCoordinator.handleSubmit': async () => {},
